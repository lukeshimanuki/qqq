...
class PickGenerator(object):
    ...
    def compute_grasp_action(self, obj, region, n_iter, pick_base_pose=None):
        grasp_params = None
        g_config = None
        ...
        for iter in range(n_iter):
            ...
            if True:#pick_base_pose is None:
                with self.robot:
                    pick_base_pose = sample_ir(obj, self.robot, self.env, region, 1)
            ...
            if g_config is not None:
                ...
                if self.problem_env.name == 'convbelt':
                    ...
                    if not self.env.CheckCollision(self.robot):
                        ...
                        two_arm_place_object(pick_action)
                        set_robot_config(self.problem_env.init_base_conf, self.robot)
                elif self.problem_env.name == 'mover':
                    #print "Sampling mover pick succeeded"
                    return g_config
                else:
                    two_arm_pick_object(obj, pick_action)
                    if not self.env.CheckCollision(self.robot):
