...
from openravepy import poseTransformPoints, matrixFromPose, matrixFromQuat, matrixFromAxisAngle, rotationMatrixFromQuat,\
  quatFromAxisAngle, poseFromMatrix, axisAngleFromRotationMatrix, quatFromRotationMatrix, quatMult, quatInverse, \
  quatRotateDirection, quatSlerp, RaveGetAffineDOFValuesFromTransform, DOFAffine, transformLookat
import numpy as np
...
def trans_transform_points(trans, points):
  ...
  #return trans.dot(np.vstack([points, np.ones([1, points.shape[1]])]))[:3,:]
...
def trans_from_point(point):
  return trans_from_pose(pose_from_quat_point(unit_quat(), np.array(point)))
...
def rot_from_axis_angle(x_angle, y_angle, z_angle): # matrixFromAxisAngle = rotationMatrixFromAxisAngle
...
def is_upright(trans):
  return within(abs(trans[2, 2]), 1) # NOTE - abs allows it to be on bottom or top face
...
def set_trans(body, trans): # I'm pretty sure this works with poses directly as well...
...
def manip_trans_from_object_trans(object_trans, grasp):
  #return np.dot(object_trans, np.linalg.inv(grasp))
