...
def record_screen(filename, delay=10):
  ...
  return run_script('osascript', os.path.join(misc_dir, 'record_screen.scpt'), filename, str(delay))

def launch_quicktime():
  misc_dir = os.path.dirname(os.path.abspath(__file__))
  return run_script('osascript', os.path.join(misc_dir, 'launch_quicktime.scpt'))
