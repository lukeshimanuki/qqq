...
def get_problem(mover):
	...
	import collections
	mconfig_type = collections.namedtuple('mconfig_type', 'operator n_msg_passing n_layers num_fc_layers n_hidden no_goal_nodes top_k optimizer lr use_mse batch_size seed num_train val_portion num_test mse_weight diff_weight_msg_passing same_vertex_model weight_initializer')
	...
	with tf.variable_scope('pick'):
		...
		pick_model = GNN(num_entities, num_node_features, num_edge_features, pick_mconfig)
	pick_model.loss_model.load_weights(pick_model.weight_file_name)
	with tf.variable_scope('place'):
		#place_model = Model(num_entities, num_node_features, num_edge_features, 1, place_mconfig)
		place_model = GNN(num_entities, num_node_features, num_edge_features, place_mconfig)
	place_model.loss_model.load_weights(place_model.weight_file_name)
	...
	while True:
		...
		if action.type == 'two_arm_pick':
			...
			for _ in range(10):
				...
				grab_obj(obj)
				newstate = State(mover, goal, state, action)
			...
			for _ in range(10):
				...
				newstate = State(mover, goal, node.state, action)
				newstate.make_pklable()
