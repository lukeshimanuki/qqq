...
def get_save_dir(parameters):
    if parameters.scenario is None:
        save_dir = './test_results/hpn_results_on_mover_domain/1/trajectory_data//'
    else:
        save_dir = './test_results/hpn_results_on_mover_domain/1/trajectory_data/special_cases/'
...
def get_raw_dir(parameters):
    if parameters.scenario is None:
        raw_dir = './test_results/hpn_results_on_mover_domain/1/'
    else:
        raw_dir = './test_results/hpn_results_on_mover_domain/1/special_cases/'
    return raw_dir
...
def get_raw_fname(parameters):
    if parameters.scenario is None:
        return str(parameters.pidx) + '.pkl'
    else:
        if parameters.scenario == 0:
            scenario = 'reachable_goal_entities'
        elif parameters.scenario == 1:
